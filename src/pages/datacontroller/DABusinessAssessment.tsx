...
const DataControllerBusinessAssessment: React.FC = () => {
...
useEffect(() => {
    if (businessPermit && businessPermit.businesses && businessPermit.businesses.length > 0) {
      const total = businessPermit.businesses.reduce(
        (sum, business) => sum + (business.capitalInvestment || 0),
        0
      );
      setTotalGross(total);
    }
    if (businessPermit && businessPermit.businesses && businessPermit.businesses.length > 0) {
      const total = businessPermit.businesses.reduce(
        (sum, business) => sum + (business.tax || 0),
        0
      );
      setTotalTax(total);
    }
...
//Data Table
const [mayorspermit] = useState(286); //Fixed
const [sanitary] = useState(100); //Fixed
const [health, setHealthPermit] = useState(0);
...
useEffect(() => {
  const fetchPermitData = async () => {
    try {
      ...
      // Check if firstName and lastName are 'none'
      if (user.firstName === 'none' && user.lastName === 'none') {
        setUserNameDisplay('ForAssessment'); // Set to 'ForAssessment' if both firstName and lastName are 'none'
      } else {
        // Concatenate firstName and lastName for the display
        const fullName = `${user.firstName} ${user.lastName}`;
        setUserNameDisplay(fullName);
      }
      
    } catch (error) {
      console.error('Error fetching permit data:', error);
      setUserNameDisplay('ForAssessment'); // Fallback value on error
    }
  };
...
return (
    ...
    <div className="DAcontent">
        ...
        <input
          type="text"
          value={dateassessed}
          readOnly // Make the input field read-only so the user cannot change it
        />
      </label>
      <label>Assessesment Status:
      <input
          type="text"
...
<div style={{ marginTop: '16px', fontWeight: 'bold' }}>
  Total Capital Investment: ₱{totalGross.toLocaleString()}
</div>
<div style={{ marginTop: '16px' }}>
<h2>Detailed Tax List:</h2>
    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
      ...
      <tbody>
        ...
        <tr>
          <td style={{ padding: '8px', borderTop: '2px solid #ddd', fontWeight: 'bold' }}>
            Total Business Computation
          </td>
          <td style={{ padding: '8px', borderTop: '2px solid #ddd', fontWeight: 'bold' }}>
        ...
        {/* Example Mayor's Permit Row */}
        <tr>
  <td style={{ padding: '8px' }}>Mayor's Permit</td>
  <td style={{ padding: '8px' }}>Fixed</td>
  <td style={{ padding: '8px' }}>₱286</td>
</tr>

        <tr>
          <td style={{ padding: '8px' }}>Sanitary Permit Fee</td>
          <td  style={{ padding: '8px' }}>Fixed</td>
        ...
        <tr>
          <td style={{ padding: '8px' }}>Zoning Clearance Fee</td>
          <td style={{ padding: '8px' }}>Fixed</td>
          <td style={{ padding: '8px' }}>₱500</td>
        </tr>

        <tr>
          <td style={{ padding: '8px' }}>Annual Inspection Fee</td>
          <td style={{ padding: '8px' }}>Fixed</td>
          <td style={{ padding: '8px' }}>₱0</td>
        </tr>

        <tr>
          <td style={{ padding: '8px' }}>Environmental Protection Fee</td>
          <td style={{ padding: '8px' }}>
          <input
          type="number"
          placeholder='---'
          onChange={(e) => setEnvironmental(e.target.value ? parseFloat(e.target.value) : 0)}
          style={{ width: '70%', padding: '4px', boxSizing: 'border-box' }}
        />
          </td>
        ...
        <tr>
          <td style={{ padding: '8px' }}>Liquor/Tobacco Plate to Sell Fee</td>
          <td style={{ padding: '8px' }}>
          <input
          type="number"
          placeholder='---'
          ...
          <td style={{ padding: '8px' }}>₱{liquortobaco}</td>
        </tr>

        <tr>
          <td style={{ padding: '8px' }}>Liquor Plate to Sell and Serve Fee</td>
          <td style={{ padding: '8px' }}>
          <input
          type="number"
          placeholder='---'
  ...
  <button className="assessbutton"onClick={handlesaveassessment}>Assess</button>
  <button  className="editbutton" onClick={() => { window.location.href = `/DAEditBusinessNature/${businessPermit?._id}`;}}>Edit Business Nature</button>
...
export default DataControllerBusinessAssessment;